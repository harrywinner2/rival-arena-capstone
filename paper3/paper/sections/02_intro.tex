...
\item \textbf{Four corrections to our own prior work}: the menu-signal result reverses
...
(\S\ref{sec:p2}); communication is \emph{not} the dominant attack surface but the
largest of several comparable levers (\S\ref{sec:p3}); and the latent-channel null we
had explained was an instrumentation artifact rather than a result
(\S\ref{sec:latent}).
\item \textbf{The effect crosses an unreadable channel} (\S\ref{sec:latent}). With the
channel verified against a token oracle and a content-mismatched control, a codec
fitted by closed-form regression on neutral text---no game data, no reward, no training
loop---reproduces the readable channel to three decimals in both games and preserves
the full $2\times2$. Every content-based defense in this paper assumes a legible
channel; this one is not, and it is cheap to build.
\item \textbf{A three-family replication} (\S\ref{sec:generality}): the market contrast
holds on Qwen2.5-14B, Llama-3.1-8B and OLMo-2-13B (pooled $-0.069$ $[-0.113,-0.026]$,
$3/3$ same sign), while the dilemma contrast is measurable on one family only---the
other two never coordinate under any arm, which we report as a capability floor.
...
Each was fixed, with a falsifier, before the run that tested it. Six were refuted by
our own data---four of them claims from our prior work, two of them hypotheses of
