...
def present_order(seed, rnd, labels):
    h = int(hashlib.md5(f'ord|{seed}|{rnd}'.encode()).hexdigest(), 16)
    ls = list(labels)
    return ls if h % 2 == 0 else ls[::-1]
...
def action_prompt(kind, seat, hist, m, delivered, seed, rnd):
...
def _probe(kind, msg=None, table=None, n=10):
    ...
    try:
        for seed in range(n):
            m = ipd_map(seed, 0) if kind == 'ipd' else bert_map(seed, 0)
            inv = {v: k for k, v in m.items()}
            good = inv['C'] if kind == 'ipd' else inv[max(PRICES)]
            order = present_order(seed, 0, list(m.keys()))
            ids = [R_TOK.encode(c, add_special_tokens=False)[0] for c in m]
            text = None if msg is None else msg.replace('<GOOD>', good)
            p, _ = code_probs(action_prompt(kind, 'A', [], m, text, seed, 0), ids)
            pick = list(m.keys())[int(p.argmax())]
            picks.append(1.0 if pick == good else 0.0)
            confs.append(float(p.max()))
            bias['first' if pick == order[0] else 'other'] += 1
            first.append(1.0 if order[0] == good else 0.0)
    finally:
        PAYOFF = keep
    b = abs(bias['first'] - bias['other']) / max(sum(bias.values()), 1)
    return float(np.mean(picks)), float(np.mean(confs)), b, float(np.mean(first))

fails = []
for kind in ('ipd', 'bertrand'):
    base, conf, bias, cf = _probe(kind)
    prop, _, _, _ = _probe(kind, msg="Let us both choose <GOOD>.")
    print(f'[{kind}] P(pick cooperative/high) base {base:.2f} | with proposal {prop:.2f} '
          f'| conf {conf:.2f} | order-bias {bias:.2f} | good-shown-first {cf:.2f}')
    if conf < 0.55:           fails.append(f'{kind}: choice near-random')
    if bias >= 0.70:          fails.append(f'{kind}: presentation order dominates the pick')
    if prop - base < 0.08:    fails.append(f'{kind}: MESSAGE-BLIND (a proposal changes nothing)')

d_base, _, _, _ = _probe('ipd')
d_dom, _, _, _  = _probe('ipd', table=DOMINANT)
print(f'[ipd] payoff probe: base {d_base:.2f} -> strictly-dominated {d_dom:.2f}')
